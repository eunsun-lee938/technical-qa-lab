...
BUG-001은 USER Lifecycle 내부의 독립 Regression Step으로 자동화하였다.
...
## 14. HTML Test Report Strategy

전체 자동화 테스트 수행 결과는
`pytest-html`을 이용하여 HTML Report로 생성한다.

HTML Report의 목적은 단순 실행 로그 보관이 아니라
전체 Regression 수행 결과를 한 화면에서 확인할 수 있도록 하는 것이다.
...
Lifecycle 내부 기능 결과는 해당 테스트의 Captured Output에서 확인한다.
...
[PASS] GET USER LIST
[CLEANUP] Deleted test user: qa_auto_xxxxxxxx
```
...
[PASS] CREATE USER
[PASS] DUPLICATE USERNAME
[FAIL] INVALID USER STATUS - AssertionError: ...
[CLEANUP] Deleted test user: qa_auto_xxxxxxxx
```

중간 Step에서 FAIL이 발생하면 이후 Lifecycle Step은 실행하지 않으며,
`finally`를 통해 생성한 테스트 데이터를 Cleanup한다.

### Report 파일

전체 자동화 수행 결과는 다음 경로에 생성한다.
...
전체 pytest Test Suite의 동작만 확인하는 경우 다음 명령을 사용한다.
...
## 16. HTML Report 운영 방침

HTML Report는 개별 테스트 개발 과정마다 생성하는 것이 아니라,
핵심 자동화 구현 및 Regression Suite 구성이 완료된 후
**전체 Test Suite를 Full Run한 결과**를 기준으로 생성한다.

개별 기능 개발 중에는 다음과 같이 특정 테스트만 실행할 수 있다.

```powershell
python -m pytest .\tests\test_users.py -v -s
```

그러나 최종 Test Report에는 특정 파일만 실행한 결과가 아니라
`tests/` 전체를 실행한 결과를 사용한다.
...
- 자동화 테스트가 실제 실행 가능한 상태임을 증빙
- 전체 Regression 결과 확인
- 인증 / 인가 / USER 관리 테스트 결과 확인
- 실패 발생 시 Assertion 및 재현 정보 확인
- Lifecycle 내부 기능 Step 수행 결과 확인
...
**수동 테스트로 식별한 핵심 위험과 실제 결함을 선정하고,  
API + DB + Audit Log를 연결한 반복 가능한 Regression Test로 전환하는 것**을 목표로 한다.

또한 USER 관리 기능은 개별 API별 테스트 데이터를 반복 생성하는 방식 대신,
하나의 테스트 데이터를 Lifecycle 전체에서 재사용하도록 구성하였다.

이를 통해 다음을 함께 확보하였다.

- 기능 검증 로직의 모듈화
- 테스트 데이터 생성 / 삭제 반복 감소
- 실제 사용자 흐름에 가까운 시나리오 구성
- 중간 실패 시 데이터 정리 보장
- 결함 Regression 반복 실행
- 자동화 코드의 재사용성과 유지보수성
